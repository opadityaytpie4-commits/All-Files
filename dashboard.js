...
let PORT = 8080; // overridden by startDashboard refs
const MAX_LOGS = 600;

// Get machine's LAN IP (first non-loopback IPv4)
function getLanIP() {
    try {
...
// ── Network ──
let _dashboardPort = 8080;
let _dashboardHost = 'localhost';
...
function startDashboard(refs) {
    ...
    // ── Network ──
    _dashboardPort = refs.dashboard_port || 8080;
    _dashboardHost = refs.dashboard_host || 'localhost';
    PORT = _dashboardPort;
    ...
    app.get('/api/network', (req, res) => {
        ...
        res.json({
            ...
            host:    _dashboardHost,
            lan_ip:  lanIp,
            is_lan:  _dashboardHost === '0.0.0.0',
            local_url: `http://localhost:${_dashboardPort}`,
            lan_url:   lanIp ? `http://${lanIp}:${_dashboardPort}` : null,
        });
    });

    app.post('/api/network', (req, res) => {
        const { host, port } = req.body || {};
        const validHosts = ['localhost', '0.0.0.0'];
        const newPort    = parseInt(port);
        if (host && !validHosts.includes(host)) return res.json({ ok: false, error: 'Invalid host. Use localhost or 0.0.0.0' });
        if (port && (isNaN(newPort) || newPort < 1024 || newPort > 65535)) return res.json({ ok: false, error: 'Port must be 1024–65535' });

        if (host) { _dashboardHost = host; if (_config) _config.dashboard_host = host; }
        if (port) { _dashboardPort = newPort; PORT = newPort; if (_config) _config.dashboard_port = newPort; }
        if (_saveConfig) _saveConfig();

        io.emit('network_update', { host: _dashboardHost, port: _dashboardPort, lan_ip: getLanIP() });
        res.json({ ok: true, host: _dashboardHost, port: _dashboardPort, restart_required: true });
    });
    ...
    server.listen(_dashboardPort, _dashboardHost, () => {
        const _R = '\x1b[0m', _B = '\x1b[1m', _CY = '\x1b[38;5;51m';
        const _GR = '\x1b[38;5;82m', _MG = '\x1b[38;5;201m', _DM = '\x1b[2m';
        const lanIp = getLanIP();
        const isLan = _dashboardHost === '0.0.0.0';
        console.log('');
        console.log(`  ${_MG}${_B}⚡ Dashboard${_R}  ${_DM}running on${_R}`);
        console.log(`  ${_DM}┌──────────────────────────────────────┐${_R}`);
        console.log(`  ${_DM}│${_R}  💻  ${_CY}${_B}http://localhost:${_dashboardPort}${_R}${' '.repeat(Math.max(0, 18 - String(_dashboardPort).length))}`);
        if (isLan && lanIp) {
            console.log(`  ${_DM}│${_R}  📱  ${_GR}${_B}http://${lanIp}:${_dashboardPort}${_R}  ${_DM}← mobile/other devices${_R}`);
        }
        console.log(`  ${_DM}└──────────────────────────────────────┘${_R}`);
        if (!isLan) {
            console.log(`  ${_DM}Only accessible on this device. Use Network mode for mobile.${_R}`);
        }
        console.log('');
    });
